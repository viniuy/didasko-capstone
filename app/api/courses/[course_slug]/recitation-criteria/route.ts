...
import { getRecitationCriteria, createCriteria } from "@/lib/services";
//@ts-ignore
export async function GET(request: Request, context: { params }) {
  try {
    ...
    const criteria = await getRecitationCriteria(course_slug);
    ...
    console.error("Error fetching recitation criteria:", error);
    return NextResponse.json(
      { error: "Failed to fetch recitation criteria", details: error.message },
      { status: 500 }
    );
  }
}
...
export async function POST(request: Request, context: { params }) {
  try {
    ...
    console.error("Error creating recitation criteria:", error);
    ...
    return NextResponse.json(
      { error: "Failed to create recitation criteria", details: error.message },
      { status: 500 }
    );
  }
}
