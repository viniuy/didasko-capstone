...
export function WheelRandomizer({
  ...
  const [studentsWithGroups, setStudentsWithGroups] = useState<StudentWithGroup[]>([]);
  ...
  const shuffleAndAssign = async () => {
    ...
    if (currentGroup.length > 0) {
      if (currentGroup.length === 1 && groups.length > 0) {
        ...
        groups[groups.length - 1].push({ ...currentGroup[0], groupNumber: groups.length });
      } else {
        // Make a smaller final group
        groups.push(currentGroup.map(s => ({ ...s, groupNumber: groups.length + 1 })));
      }
    }
  ...
  const createGroups = async () => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to create groups");
    } finally {
      setIsCreating(false);
    }
  };
  ...
  return (
    <>
      <button
        ...
        className="relative flex flex-col items-center justify-center rounded-full bg-gray-200 hover:bg-gray-300 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-lg border-2 border-gray-200">
              <label className="text-sm font-semibold text-[#124A69]">
                Students per group:
              </label>
              <Input
                type="number"
              ...
              <span className="text-sm text-gray-600">
                {availableStudents.length} students → ~{Math.floor(availableStudents.length / groupSize)} groups
              </span>
            </div>

            {/* Student Cards Grid */}
            <div className="min-h-[400px] bg-gray-50 rounded-lg p-6">
              <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 ${isShaking ? 'animate-shake' : ''}`}>
                {studentsWithGroups.map((student, idx) => (
                  <div
                    ...
                    className={`
                      ...
                      ${currentAssigning === idx ? 'ring-4 ring-[#124A69] ring-offset-2 scale-110' : ''}
                  ...
                  >
                    {student.groupNumber && (
                      <div className="absolute -top-2 -right-2 w-8 h-8 bg-white text-[#124A69] rounded-full flex items-center justify-center text-xs font-bold shadow-lg border-2 border-[#124A69]">
                        {student.groupNumber}
                      </div>
                    )}
                    <div className={`text-sm font-medium truncate ${student.isAssigned ? 'text-white' : 'text-gray-700'}`}>
                      {student.name}
                    </div>
                    {student.isAssigned && (
                      <div className="mt-1 text-xs text-white/90 font-semibold">
            ...
            {assignedGroups.length > 0 && (
              <div className="bg-gray-50 rounded-lg p-4 border-2 border-[#124A69]">
                <h3 className="font-bold text-[#124A69] mb-3 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 max-h-[200px] overflow-y-auto">
                  {assignedGroups.map((group, idx) => (
                    <div key={idx} className="bg-white rounded-lg p-3 border-2 border-gray-200 shadow-sm hover:border-[#124A69]/30 transition-colors">
                      <div className="font-semibold text-sm mb-2 text-[#124A69] flex items-center gap-2">
                        <div className="w-6 h-6 bg-[#124A69] text-white rounded-full flex items-center justify-center text-xs">
                          {idx + 1}
                        </div>
                        Group {idx + 1}
                      </div>
                      <div className="space-y-1">
                        {group.map((s, sIdx) => (
                          <div key={s.id} className="text-xs text-gray-600 flex items-center gap-1">
                            <span className="text-gray-400">{sIdx === 0 ? "👑" : "•"}</span>
              ...
              {assignedGroups.length === 0 ? (
                <>
                  ...
                  <Button
                    ...
                    className="flex-1 bg-[#124A69] hover:bg-[#0d3a56] text-white font-semibold"
                  >
                    {isShaking ? (
                      <>
                ...
                <>
                  ...
                  <Button
                    ...
                    className="flex-1 bg-[#124A69] hover:bg-[#0d3a56] text-white font-semibold"
                  >
                    {isCreating ? "Creating Groups..." : `Create ${assignedGroups.length} Groups 🎉`}
                  </Button>
                </>
              )}
            </div>
